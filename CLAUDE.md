...
- Sistema de indicação (`affiliates`) tem link/cliques/cadastros funcionando, mas **não calcula recompensa em dinheiro automaticamente** — `totalEarned`/`totalPaid` ficam em 0, só a chave PIX é coletada. É de propósito: automatizar comissão mexeria em código de pagamento, mesma régua da seção de pagamento acima ("não mexer sem o dono pedir"). Quando o valor da recompensa for decidido, ligar o cálculo em `POST /api/payment` (mesmo lugar que já incrementa `totalSignups` — ver `src/lib/db/affiliates.ts`).
