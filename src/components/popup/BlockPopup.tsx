...
export function BlockPopup({ block, onClose }: BlockPopupProps) {
  ...
  // Filtra redes que foram preenchidas
  const activeSocials = SOCIAL_CONFIG.filter(s => !!(block as any)[s.key])
    ...
    >
      <div className="popup-enter relative w-full max-w-sm overflow-hidden shadow-2xl sm:rounded-2xl" style={{ background: '#111', maxHeight: '90vh', overflowY: 'auto', borderRadius: '16px 16px 0 0' }}>
        ...
        <div className="p-4 pt-12">
          ...
          {/* Se não tem redes, mostra o Instagram direto */}
          {activeSocials.length === 0 && (
            <div className="mb-4">
              <a
                href={igUrl}
              ...
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4.5"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg>
