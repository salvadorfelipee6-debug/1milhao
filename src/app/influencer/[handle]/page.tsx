...
const SOCIAL_CONFIG = [
  ...
  { key: 'twitterUrl',    label: 'X / Twitter', color: '#ffffff', bg: 'rgba(255,255,255,0.06)', border: 'rgba(255,255,255,0.15)',
    icon: <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg> },
  ...
  { key: 'kwaiUrl',       label: 'Kwai',       color: '#ff8c00', bg: 'rgba(255,140,0,0.08)',    border: 'rgba(255,140,0,0.25)',
    icon: <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18"><path d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm4.5 10.5l-6 3.5V9l6 3.5z"/></svg> },
...
export default async function InfluencerProfilePage({ params }: Props) {
  ...
  const activeSocials = SOCIAL_CONFIG.filter(s => {
    const blockKey = s.key as string
    return !!(block as any)[blockKey]
  })
  // Instagram sempre aparece
  const igSocial = { key: 'instagramUrl', label: 'Instagram', color: '#e1306c',
    bg: 'rgba(225,48,108,0.12)', border: 'rgba(225,48,108,0.3)',
    url: igUrl, icon: SOCIAL_CONFIG[0].icon }
          ...
          {(activeSocials.length > 0 || true) && (
            <div className="mb-3 space-y-2">
              ...
              >
                ...
                <span className="flex-1 text-sm font-semibold">Instagram</span>
                <span className="text-xs opacity-50">↗</span>
              </a>

              {activeSocials.filter(s => s.key !== 'instagramUrl').map(s => (
                <a
                  key={s.key}
